...
async function buildSite() {
  ...
  // ----------------------------------------------------
  // 1. GENERATE INDIVIDUAL SEO POST PAGES (Flat File Method)
  // ----------------------------------------------------
...
<body>
    ...
    <script>
        function openChat(ownerId, postId) {
            ...
            if(!token) {
                // Save where the user wanted to go so auth can send them back later
    ...
    // The Flat File Method: Keeps your GitHub repo clean by generating single .html files
    // GitHub Pages will automatically serve this without the .html extension when linked correctly
...
<body>
    ...
    <script>
        // EMBEDDED DATA FROM BUILD STEP (Zero DB Reads on Visit!)
        ...
        function renderListings(postsToRender, isFallback = false, fallbackArea = '') {
            ...
            postsToRender.forEach(post => {
                ...
                // Absolute root link pointing to the extensionless path generated by Flat File Method
  ...
  console.log(`Successfully built site with ${postsData.length} listings using Flat File method.`);
}
